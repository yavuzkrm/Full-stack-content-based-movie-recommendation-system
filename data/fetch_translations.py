...
def fetch_and_save_movie_translations():
    """Turkish title + overview for every movie that doesn't have one yet.
    ...
    try:
        ...
        for movie_id in movie_ids:
            try:
                ...
                data = res.json()
                # TMDB returns the ENGLISH title/overview back as a fallback
                # if it has no Turkish translation for this specific movie —
                # storing that would defeat the whole point (title_tr would
                # just silently duplicate title, and the language toggle
                # would look broken for that movie without ever knowing why).
                # Only save it if TMDB actually gave us something back AND
                # that something isn't just an empty string.
