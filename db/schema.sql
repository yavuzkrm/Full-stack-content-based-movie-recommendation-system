...
CREATE TABLE IF NOT EXISTS movies (
    ...
    overview    TEXT,
    -- Turkish title/overview, used for two different things:
    --   1. /api/search also matches against title_tr, so searching "Dövüş
    --      Kulübü" finds "Fight Club" even though the SITE still only ever
    --      shows the English title/overview in search results.
    --   2. When the site's language toggle (see index.html) is set to
    --      Turkish, movie titles/overviews actually DISPLAY from these
    --      columns instead — falling back to the English ones for any movie
    --      that hasn't been translated yet (see data/fetch_translations.py).
    -- Both are nullable on purpose: a movie can exist here with NULL
    -- title_tr/overview_tr until fetch_translations.py gets around to it —
    -- nothing else in the app breaks in the meantime, it just falls back to
    -- English for that one movie.
...
CREATE TABLE IF NOT EXISTS genres (
    ...
    name_tr VARCHAR(100)  -- Turkish genre name, same fallback-to-English convention as movies.title_tr above
);
