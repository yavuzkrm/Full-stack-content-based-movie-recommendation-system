...
from lists.movies_lists import movies_by_genre, top_250, get_all_genres
from lists.people_lists import movies_by_cast_member, movies_by_director
...
def api_search():
    """Powers the search-as-you-type box. Matches any movie whose ENGLISH OR
    TURKISH title CONTAINS the query — typing "Dövüş Kulübü" finds "Fight
    Club" this way, even though the site only ever displays the movie under
    whichever title matches the current language toggle (see title_tr on
    movies in db/schema.sql for why) — then sorts so that titles STARTING
    WITH the query show up first (typing "int" should surface "Interstellar"
    before some movie that merely mentions "interior" in the middle of its
    title), and breaks any remaining ties by rating."""
    ...
    title_tr_lower = df["title_tr"].str.lower()

    # A movie matches if EITHER its English or Turkish title contains the
    # query — e.g. searching "dövüş" only matches via title_tr, searching
    # "fight" only matches via title, and both stay independent of whichever
    # language the site's toggle is currently showing.
    mask = title_lower.str.contains(q_lower, regex=False) | title_tr_lower.str.contains(q_lower, regex=False)
    matches = df[mask].copy()

    # "Starts with" is checked against WHICHEVER title actually matched, so a
    # Turkish query that starts-with title_tr still gets the same "exact
    # prefix beats mid-title match" boost that an English query gets against
    # title — check each column separately.
    starts_en = title_lower[mask].str.startswith(q_lower)
    starts_tr = title_tr_lower[mask].str.startswith(q_lower)
    matches["_starts_with"] = (starts_en | starts_tr).values
    matches = matches.sort_values(by=["_starts_with", "display_rating"], ascending=[False, False])

    cols = ["id", "title", "title_tr", "poster_path", "display_rating"]
...
def api_get_top_250():
    return df_to_json_safe(top_250())

# --- Movies by genre ------------------------------------------------------------
@app.route("/api/genre/<string:genre_name>")
def api_get_movies_by_genre(genre_name):
    return df_to_json_safe(movies_by_genre(genre_name))

# --- All genre names (for a "browse by genre" UI, English + Turkish name each) --
@app.route("/api/genres")
def api_get_all_genres():
    return jsonify(get_all_genres())

# --- Movies by cast member -------------------------------------------------------
@app.route("/api/cast/<string:person_name>")
def api_get_movies_by_cast_member(person_name):
    return jsonify(movies_by_cast_member(person_name))

# --- Movies by director -------------------------------------------------------
@app.route("/api/director/<string:person_name>")
def api_get_movies_by_director(person_name):
    return jsonify(movies_by_director(person_name))
