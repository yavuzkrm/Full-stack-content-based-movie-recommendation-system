...
def load_data():
    ...
    cursor.execute("""
        SELECT id, title, COALESCE(title_tr, title) AS title_tr,
               overview, COALESCE(overview_tr, overview) AS overview_tr,
               vote_avg, vote_count, poster_path
        FROM movies
    """)
    ...
    cursor.execute("""
        SELECT mg.movie_id, GROUP_CONCAT(g.name SEPARATOR ',') AS genres,
               GROUP_CONCAT(COALESCE(g.name_tr, g.name) SEPARATOR ',') AS genres_tr
    ...
    df["genres_tr"] = df["genres_tr"].fillna("")
    df["overview"] = df["overview"].fillna("")
    df["overview_tr"] = df["overview_tr"].fillna("")  # COALESCE in the SQL above already covers most cases, but a movie with NO English overview at all (both NULL) still needs this
...
RESULT_COLUMNS = ["id", "title", "title_tr", "genres", "genres_tr", "display_rating",
                  "overview", "overview_tr", "poster_path", "directors", "cast"]
