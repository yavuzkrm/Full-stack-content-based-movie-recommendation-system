...
outrank a movie 50,000 people actually watched and loved. The same engine
...
- 🔎 **Search-as-you-type**, ranked so titles that *start with* your query
  ...
  — and matches against the Turkish title too, so searching "Dövüş Kulübü"
  finds *Fight Club* even though the site still displays it under whichever
  title matches the current language.
- 🏆 **Top 250** and **browse-by-genre** pages, both sorted by the same
  Bayesian quality score the recommendation engine itself uses (see below) —
  not just raw average rating.
- 🎭 **Clickable cast, directors and genres** — click a genre tag or a
  person's name anywhere a movie is shown and it takes you straight to a
  filmography/genre-browsing page built from the same catalogue.
- 🇬🇧🇹🇷 **English/Turkish language toggle** — flips movie titles, overviews
  and genre tags between the two languages app-wide; falls back to English
  automatically for any movie that hasn't been translated yet.
...
The same blended score also powers **Top 250** and **browse-by-genre**
(`lists/movies_lists.py`) and a person's filmography (`lists/people_lists.py`)
— all three are just different filters/sorts over the same cached data the
recommendation engine already builds, not separate models.

## 🇬🇧🇹🇷 English/Turkish support

Every movie carries an optional Turkish title/overview (`title_tr`,
`overview_tr` on `movies`; `name_tr` on `genres` — see `db/schema.sql`),
filled in by `data/fetch_translations.py`. Two independent things read
these columns:

- **Search always checks both languages**, regardless of which one the site
  is currently displaying — `/api/search` matches a query against `title`
  OR `title_tr`, so a Turkish query finds an English-titled movie.
- **The language toggle** in the header (`state.lang` in `templates/index.html`)
  switches which column every card/detail view actually *displays*. A movie
  that hasn't been translated yet just falls back to English automatically
  (`COALESCE(title_tr, title)` at the SQL level in `recommend/engine.py`) —
  nothing in the frontend needs its own fallback logic.
...
│   └── fetch_translations.py       # fills in Turkish title/overview/genre names — safe to re-run any time
├── recommend/
│   └── engine.py                   # the recommendation model itself
├── lists/
│   ├── movies_lists.py             # Top 250 / browse-by-genre queries
...
# 5. Fetch Turkish titles/overviews/genre names for that catalogue
...
- Missing data doesn't crash the engine — a signal with no data (e.g.
  ...
  the blend until real data is there. The same goes for translations: a
  movie without `title_tr`/`overview_tr` yet just displays in English even
  with the language toggle set to Turkish.
