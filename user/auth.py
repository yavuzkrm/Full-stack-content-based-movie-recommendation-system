...
def get_all_ratings(user_id):
    """Fetches all ratings for a specific user. Returns a list of dictionaries
    containing the movie ID and rating."""
    conn = get_db()
    cursor = conn.cursor(dictionary=True)
    try:
        cursor.execute(
...
def delete_rating(user_id, movie_id):
    _remove_from_list("user_ratings", user_id, movie_id)


def get_rated_movies(user_id):
    return _get_list_with_movie_details("user_ratings", user_id, "DESC")
